...
<style>
    ...
    .best-time{background:linear-gradient(90deg, #0f2027, #203a43, #2c5364);padding:10px;border-radius:5px;text-align:center;border:1px solid #00ffcc;margin-bottom:15px;}
</style>
...
def play_sound():
    st.components.v1.html(
        ...
        oscillator.start();
        setTimeout(function(){ oscillator.stop(); }, 250);
        </script>""", height=0
    )
...
def sessoes_mercado():
    ...
    # Lógica de Horários de Alta Precisão (Melhoria Solicitada)
    now = datetime.now()
    hour = now.hour
    if 10 <= hour <= 14:
        q, qc = ("💎 ALTA PRECISÃO (LONDRES + NY)", "#00ffcc")
    elif 5 <= hour <= 9:
        q, qc = ("✅ BOM (ABERTURA LONDRES)", "#00ff88")
    elif 19 <= hour <= 23:
        q, qc = ("🟡 MÉDIO (ÁSIA)", "#ffaa00")
    else:
        q, qc = ("🔴 BAIXA VOLATILIDADE", "#ff4444")
...
# Barra de Horário em Tempo Real (Melhoria Integrada)
timing, t_q, t_color = sessoes_mercado()
st.markdown(f'<div class="best-time">SITUAÇÃO DO MERCADO AGORA: <b style="color:{t_color}">{t_q}</b></div>', unsafe_allow_html=True)
...
with t1:
    ...
    st.markdown(f'<div class="sess-box" style="border-left:4px solid {s_qc}"><b>Sessões Ativas:</b> {s_at}</div>', unsafe_allow_html=True)
    ...
    if st.session_state.analisado:
        ...
        for idx, nome in enumerate(st.session_state.resultados):
            ...
            with cols[idx % 3]:
                ...
                # DISPARO DE ALERTAS (BIP + TOAST)
                if sc_c >= 70 or sc_p >= 70:
                    play_sound() # Bip via JS
                    tipo = "CALL ▲" if sc_c >= sc_p else "PUT ▼"
                    cor = "#00ff88" if "CALL" in tipo else "#ff4444"
                    scr = max(sc_c, sc_p)
                    
                    # Notificação Toast de Sistema
                    st.toast(f"🚨 Sniper: {nome} - {tipo} ({scr}%)", icon="🎯")
